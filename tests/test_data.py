...
def load_data():
    ...
    train_loader = Loader().load(hyperparameters, batch_amount=hyperparameters.training_batch, folder_path="data/processed/train")
    validation_loader = Loader().load(hyperparameters, batch_amount=hyperparameters.validation_batch, folder_path="data/processed/val")
...
def validate_dataset(loader, N_samples, data_shape, num_labels, dataset_type = "Training"):
    ...
    for batch, labels in loader:
        ...
        for sample in batch:
            assert list(sample.shape) == data_shape, f"Data shape in {dataset_type} dataset is {sample.shape}, expected {data_shape}"

        # Add labels to the unique labels set
        unique_labels.update(labels.tolist())

    # Check if all labels are represented
    assert len(unique_labels) == num_labels, f"Number of unique labels in {dataset_type} dataset are {len(unique_labels)}, expected {num_labels}"
